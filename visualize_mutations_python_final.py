...
import argparse
from virus_mapping import get_virus_info, VIRUS_DATABASE
import sys
import os
...
# Gene coordinates will be loaded dynamically based on virus
...
def map_position_to_gene(position, accession):
    ...
    gene_coords = virus_info['gene_coords']
    
    for gene, (start, end) in gene_coords.items():
        if start <= position <= end:
            return gene
    return None
def setup_gene_visualization(accession):
    """Setup gene coordinates and colors for visualization"""
    virus_info = get_virus_info(accession)
    gene_coords = virus_info.get('gene_coords', {})
    
    # Default colors for common gene types
    default_colors = {
        'C': '#4575b4',
        'prM': '#74add1', 
        'Env': '#abd9e9',
        'E': '#abd9e9',  # Envelope for alphaviruses
        'E1': '#abd9e9',
        'E2': '#7fb3d3',
        'E3': '#5ba5c9',
        'NS1': '#fdae61',
        ...
        'NS4b': '#9970ab',
        'NS5': '#c2a5cf',
        'nsP1': '#fdae61',  # Non-structural proteins for alphaviruses
        'nsP2': '#f46d43',
        'nsP3': '#d73027',
        'nsP4': '#a50026'
    }
    ...
    for gene in gene_coords.keys():
        gene_colors[gene] = default_colors.get(gene, '#cccccc')
    
    # Determine structural vs non-structural genes
    structural_genes = []
    nonstructural_genes = []
    
    for gene in gene_coords.keys():
        if gene in ['C', 'prM', 'Env', 'E', 'E1', 'E2', 'E3']:
