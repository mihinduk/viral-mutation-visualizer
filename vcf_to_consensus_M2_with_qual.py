...
Examples:
  ...
  %(prog)s -i variants.vcf.gz -r reference.fa --validate-vcf
    ...
    parser.add_argument("--min-qual", type=float, default=None,
                       help="Minimum QUAL score to include variants (default: no filtering)")
    ...
    # Apply QUAL filtering if requested
    if args.min_qual is not None:
        filtered_vcf = compressed_vcf.with_suffix('.filtered.vcf.gz')
        success, error = filter_vcf_by_qual(compressed_vcf, filtered_vcf, args.min_qual)
        if not success:
            print(f"Error: Failed to filter VCF by QUAL score: {error}")
            if temp_compressed and not args.keep_temp:
                compressed_vcf.unlink()
            sys.exit(1)
        
        # Use filtered VCF for consensus
        compressed_vcf = filtered_vcf
        
        # Index the filtered VCF
        print("
Indexing filtered VCF...")
        success, output, error = index_vcf(compressed_vcf, force=True)
        if not success:
            print(f"Error: Failed to index filtered VCF: {error}")
            sys.exit(1)
